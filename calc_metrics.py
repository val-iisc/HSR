...
@click.option('--trunc_psi', help='To apply truncation trick', type=float, default=1.0)


def calc_metrics(ctx, network_pkl, metrics, data, mirror, gpus, verbose, numgen, clean, trunc_psi):
    """Calculate quality metrics for previous training run or pretrained network pickle.
    ...
    # Validate arguments.
    args = dnnlib.EasyDict(metrics=metrics, num_gpus=gpus, network_pkl=network_pkl, verbose=verbose, clean=clean, trunc_psi=trunc_psi)
