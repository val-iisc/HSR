...
# This function will compare images for all the set of face attributes 
def compare_pairs(img1_id, img2_id, attrs, logits_vars, attr_name):
    attr_set1 = attrs[img1_id]
    attr_set2 = attrs[img2_id]

    delta = attr_set1 - attr_set2 
    delta_normed = delta / logits_vars # Norm

    attr_id = get_attr_id(attr_name) # Attr id
    delta_acc = np.sum(delta_normed) - delta_normed[attr_id] # delta by removing attr_id
...
# Rescore the models by evaluating the misclassification for all the other attributes after changin one attribute 
def create_confusion_matrix_row(img1_id, img2_id, attrs, logits_vars, attr_list, attr_name):
    attr_set1 = attrs[img1_id]
    attr_set2 = attrs[img2_id] 

    delta = attr_set1 - attr_set2 
    delta_normed = delta / logits_vars # Norm for the difference in deltas 
    

    attr_list_ids = [] # Selecting only a few given set of attributes to compare  
    for id in range(0, len(attr_list)):
        attr_id = get_attr_id(attr_list[id])
        attr_list_ids.append(attr_id)

    attr_deltas = []
    for id in range(0, len(attr_list_ids)):
        attr_delta = attr_list_ids[id] 

    return attr_delta

# This function will create confision matrix for a given set of rows and 
def create_confusion_matrix(img1_id, img2_id, attrs, logits_vars, attr_list):
    rows = []
    for at_nm in attr_list:
        row = create_confusion_matrix(img1_id, img2_id, attrs, logits_vars, attr_list, at_nm) # computing attr rows one by one 
        rows.append(row)
...
def run_main():
    fld_path = 'results/ours/results.pkl'
    save_prefix = 'results/ours'
    delta_accum_ours, delta_accum_ours_stds, attr_list = process_results(fld_path, save_prefix=save_prefix, attach_mean_to_fname=True)

    fld_path = 'results/baseline/results.pkl'
    save_prefix = 'results/baseline'
    delta_accum_baseline, delta_accum_baseline_stds, attr_list = process_results(fld_path, save_prefix=save_prefix, attach_mean_to_fname=True) 

    save_prefix = 'results/rebuttal/mean_graphs'
    process_outputs(delta_accum_ours, delta_accum_baseline, delta_accum_ours_stds, delta_accum_baseline_stds, attr_list, 'no-log', save_prefix=save_prefix)   

    compute_metrics(delta_accum_ours, delta_accum_baseline, delta_accum_ours_stds, delta_accum_baseline_stds)  
