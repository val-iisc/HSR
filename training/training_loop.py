...
from training.get_moments import *
...
#----------------------------------------------------------------------------
# def init_head(G_ema, res):
#     data = get_data(G_ema, res)
#     train_data, test_data = split_data(data)
    
#     train_X, train_y = train_data
#     test_X, test_y = test_data

#     lin_reg = LinearRegressor().fit(train_X, train_y)
#     train_loss = lin_reg.score(train_X, train_y)
#     test_loss = lin_reg.score(test_X, test_y)
...
def training_loop(
    ...
    G_kwargs.synthesis_kwargs.pred_feat_dims = PretrainedFE(loss_kwargs.model_name, torch.device('cpu')).layer_dims
    ...
    if (resume_pkl is not None and exact_resume > 0):
        with dnnlib.util.open_url(resume_pkl) as f:
            resume_data = legacy.load_network_pkl(f)
    ...
    for name, module, opt_kwargs, reg_interval in phase_list:
        head_params = list(map(lambda x: x[1],list(filter(lambda kv: 'feat_transfer_list' in kv[0], module.named_parameters()))))
        base_params = list(map(lambda x: x[1],list(filter(lambda kv: 'feat_transfer_list' not in kv[0], module.named_parameters()))))
        head_lr_mult = 1.
        if reg_interval is None:
            paramts = [{'params': base_params}, {'params': head_params, 'lr': opt_kwargs.lr*head_lr_mult}] if len(head_params) else [{'params': base_params}]
            opt = dnnlib.util.construct_class_by_name(params=paramts, **opt_kwargs) # subclass of torch.optim.Optimizer
    ...
    while True:
        ...
        for phase, phase_gen_z, phase_gen_c in zip(phases, all_gen_z, all_gen_c):
            ...
            for round_idx, (real_img, real_c, gen_z, gen_c) in enumerate(zip(phase_real_img, phase_real_c, phase_gen_z, phase_gen_c)):
                ...
                #if phase.name == 'Gmain':
                #    res = [8,16,32,64]
                #    _ = [training_stats.report('Grads/G_head_'+ str(r), f.in_grad_norm) for f, r in zip(phase.module.synthesis.feat_transfer_list, res)]
                #    _ = [f.handle.remove() for f in phase.module.synthesis.feat_transfer_list if hasattr(f, 'handle')]
        ...
        if ((network_snapshot_ticks is not None ) and (done or cur_tick % network_snapshot_ticks == 0) ):
            ...
            # Susmit
            # snapshot_data['loss'] = loss.get_loss_state()
